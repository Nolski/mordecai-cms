(function() {
            ...
            response.json().then(function(json) {
                ...
                $(".js-embed").on("click", function(e) {
                    ...
                    var val =
                        '<iframe src="' +
                        url +
                        '" frameborder="0" style="border: none;" width="100%" height="780;"></iframe>';
    ...
    var TimeMapperView = Backbone.View.extend({
        ...
        initialize: function(options) {
            ...
            this.timelineState = _.extend(
                {},
                this.datapackage.tmconfig.timeline,
                {
                    nonUSDates: this.datapackage.tmconfig.dayfirst,
                    timelineJSOptions: _.extend(
                        {},
                        this.datapackage.tmconfig.timelineJSOptions,
                        {
                            hash_bookmark: true
                        }
                    )
            ...
            this.model.fetch().done(function() {
                debugger;
                self.model
                    .query({ size: self.model.recordCount })
                    .done(function() {
                        self._dataChanges();
        ...
        _setStartPosition: function() {
            ...
            this.timelineState.timelineJSOptions = _.extend(
                this.timelineState.timelineJSOptions,
                {
        ...
        _dataChanges: function() {
            ...
            this.model.records.each(function(record) {
                ...
                var startDate = Date.parse(
                    normalizeDate(
                        record.get("start"),
                        self.datapackage.tmconfig.dayfirst
                    )
                );
                var data = {
                    ...
                    mediacaption: record.get("caption") ||
                        record.get("mediacaption") ||
                        record.get("imagecaption"),
                    mediacredit: record.get("imagecredit") ||
                        record.get("mediacredit")
                };
            ...
            var minOpacity = 0.3, opacityRange = 1.0 - minOpacity;
            this.model.records.each(function(rec) {
                var temporalRangeLocation =
                    (rec.get("startParsed") - minDate) / dateRange;
                rec.set({
        ...
        _setupTimeline: function() {
            ...
            this.timeline.convertRecord = function(record, fields) {
                ...
                if (!out) {
                    if (typeof console !== "undefined" && console.warn) {
                        console.warn(
                            "Failed to extract timeline entry from record",
                            record.toJSON()
                        );
                    }
                    return null;
        ...
        _setupMap: function() {
            ...
            this.map.geoJsonLayerOptions.pointToLayer = function(
                feature,
                latlng
            ) {
                ...
                var maxSize = 400;
                var radius = parseInt(
                    Math.sqrt(maxSize * recordAttr.relativeSize)
                );
                if (radius) {
                ...
                var label =
                    recordAttr.title + "<br />Date: " + recordAttr.start;
                if (recordAttr.size) {
